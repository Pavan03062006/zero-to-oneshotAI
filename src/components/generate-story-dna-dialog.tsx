...
import { generateStoryDna, STATUS_COPY, StoryEngineError, type GenerateStoryDnaResult } from "@/lib/story-engine";
...
export function GenerateStoryDnaDialog({ projectId }: { projectId: string }) {
  ...
  const mut = useMutation({
    mutationFn: () => generateStoryDna({ projectId, creativeDirection: direction.trim() || undefined }),
    onSuccess: (data) => {
      setResult(data);
  ...
  const handleClose = (next: boolean) => {
    if (mut.isPending) return;
    setOpen(next);
    if (!next) { setResult(null); mut.reset(); }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      ...
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle className="uppercase tracking-wide flex items-center gap-2"><Wand2 className="h-5 w-5" /> Generate Story DNA</DialogTitle>
          <DialogDescription>
            The story engine drafts characters, locations, events, and a Story Bible from your project premise. Everything lands as <span className="text-foreground font-medium">Proposed canon</span> — nothing is auto-approved.
          </DialogDescription>
        </DialogHeader>
        ...
        {result ? (
          <div className="space-y-3 py-2">
            <div className="rounded-3xl border border-border/60 bg-card/60 p-5">
              <div className="text-xs uppercase tracking-wider text-muted-foreground">Fresh material</div>
              ...
              {result.document?.title && (
                <div className="text-xs text-muted-foreground mt-3">
                  Story Bible drafted: <span className="text-foreground">{result.document.title}</span>
                </div>
              )}
              <div className="text-xs text-muted-foreground mt-4">
                All new material is badged <span className="text-foreground">PROPOSED</span> until you accept it into canon.
              </div>
            </div>
          </div>
        ) : (
        ...
        <DialogFooter>
          {result ? (
            ...
            <>
              <Button variant="ghost" onClick={() => handleClose(false)} disabled={mut.isPending}>Cancel</Button>
              <Button onClick={() => mut.mutate()} disabled={mut.isPending} className="gap-2 uppercase tracking-wider">
                {mut.isPending ? <><Loader2 className="h-4 w-4 animate-spin" /> Generating</> : <><Sparkles className="h-4 w-4" /> Generate</>}
              </Button>
            </>
          )}
        </DialogFooter>
