...
import { listDocuments, updateDocument, createDocument, listIssues, getDocument, createRevision } from "@/lib/queries";
...
import { Plus, Sparkles, ShieldAlert, Check, Loader2 } from "lucide-react";
...
function Chapters() {
  ...
  const create = useMutation({
    mutationFn: () => createDocument(id, `Chapter ${(docs.data?.length ?? 0) + 1}`, (docs.data?.length ?? 0) + 1),
    onSuccess: (d) => { qc.invalidateQueries({ queryKey: ["docs", id] }); setSelected(d.id); toast.success("Chapter added"); },
    ...
    <div className="grid gap-4 lg:grid-cols-[240px_1fr_280px] min-h-[70vh]">
      <aside className="space-y-1">
        ...
        {list.length === 0 ? (
          ...
          <button key={d.id} onClick={() => setSelected(d.id)} className={`w-full text-left px-3 py-2 rounded-md text-sm border transition ${selected === d.id ? "border-primary bg-primary/10 text-primary" : "border-transparent hover:bg-card/60 text-muted-foreground hover:text-foreground"}`}>
      ...
      {selected ? <Editor key={selected} docId={selected} /> : (
        ...
        {issues.error ? (
          ...
          <div key={i.id} className="rounded-md border border-border/60 bg-card/60 p-3">
            <div className="flex items-center justify-between">
              <Badge variant={i.severity === "critical" ? "destructive" : "outline"} className="text-[10px]">{i.severity}</Badge>
              <span className="text-[10px] text-muted-foreground">{i.status}</span>
            </div>
            <div className="text-sm font-medium mt-1">{i.title}</div>
            {i.explanation && <p className="text-xs text-muted-foreground mt-1 line-clamp-3">{i.explanation}</p>}
          </div>
        ))}
      </aside>
    </div>
...
function Editor({ docId }: { docId: string }) {
  ...
  useEffect(() => {
    if (q.data) { setTitle(q.data.title); setContent(q.data.content); }
  }, [q.data]);
  ...
  return (
    <div className="rounded-xl border border-border/60 bg-card/40 p-5 flex flex-col">
      <div className="flex items-center justify-between gap-3">
        <Input value={title} onChange={(e) => setTitle(e.target.value)} className="font-serif text-xl border-0 shadow-none px-0 focus-visible:ring-0 bg-transparent" />
        <div className="flex items-center gap-2 shrink-0">
          <span className="text-xs text-muted-foreground flex items-center gap-1">
            {state === "saving" ? <><Loader2 className="h-3 w-3 animate-spin" /> Saving</> : state === "saved" ? <><Check className="h-3 w-3 text-primary" /> Saved</> : "Idle"}
          </span>
          <ContinuityScanButton projectId={projectId} documentId={docId} getContent={() => content} size="sm" variant="outline" label="Run continuity scan" />
          <DropdownMenu>
            <DropdownMenuTrigger asChild><Button size="sm" variant="outline" className="gap-1.5"><Sparkles className="h-3.5 w-3.5" /> AI assist</Button></DropdownMenuTrigger>
            <DropdownMenuContent align="end" className="w-52">
              <DropdownMenuLabel>In-scene assists</DropdownMenuLabel>
              <DropdownMenuItem onClick={() => toast.message("Rewriting for tone…")}>Rewrite for tone</DropdownMenuItem>
              <DropdownMenuItem onClick={() => toast.message("Tightening prose…")}>Tighten prose</DropdownMenuItem>
              <DropdownMenuItem onClick={() => toast.message("Continuing scene…")}>Continue scene</DropdownMenuItem>
              <DropdownMenuSeparator />
              <DropdownMenuItem onClick={() => toast.message("Scanning canon…")}>Check against canon</DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        </div>
      </div>
