    ...
    <div className="grid gap-4 lg:grid-cols-[240px_1fr]">
      <aside className="space-y-1">
        ...
        {docs.data.map((d) => (
          <button
            ...
            className={`w-full text-left px-3 py-2 rounded-md text-sm border transition ${active === d.id ? "border-primary bg-primary/10 text-primary" : "border-transparent hover:bg-card/60 text-muted-foreground hover:text-foreground"}`}
          >
            {d.title}
          </button>
        ))}
      ...
      <div className="space-y-3">
        {revs.isLoading ? (
          ...
          <div className="rounded-xl border border-destructive/40 bg-destructive/10 p-6 text-sm">
            Unable to load revisions. Refresh to retry.
          </div>
        ) : (revs.data ?? []).length === 0 ? (
          <div className="rounded-xl border border-dashed border-border/60 p-10 text-center text-sm text-muted-foreground">
