...
function Continuity() {
  ...
  const filtered = useMemo(
    () =>
      (q.data ?? []).filter(
        (i) => (sev === "all" || i.severity === sev) && (st === "all" || i.status === st),
      ),
    [q.data, sev, st],
  );
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <Select value={docId} onValueChange={setDocId}>
            <SelectTrigger className="min-w-[220px]">
              <SelectValue
                placeholder={docs.isLoading ? "Loading chapters…" : "Select a chapter"}
              />
            </SelectTrigger>
            <SelectContent>
              {(docs.data ?? []).map((d) => (
      ...
      <CardContent className="p-5">
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0 flex-1">
            ...
            {issue.explanation && (
              <p className="text-sm text-muted-foreground mt-2">{issue.explanation}</p>
            )}
            {issue.suggested_fix && (
              ...
              <Collapsible open={open} onOpenChange={setOpen} className="mt-3">
                <CollapsibleTrigger asChild>
                  <Button
                    variant="ghost"
                    size="sm"
                    className="gap-1.5 -ml-2 uppercase tracking-wider text-xs"
                  >
                    <ChevronDown
                ...
                <CollapsibleContent className="space-y-2 mt-2">
                  {evidence.map((ev, idx) => (
                    <div
                      key={idx}
                      className="rounded-2xl border border-border/60 bg-background/60 p-3 text-xs space-y-2"
                    >
                      {ev.source && (
                        <div>
                          ...
                          <div className="text-foreground/90 mt-0.5">{ev.source}</div>
                        </div>
                      )}
                      {ev.quote && (
                        <div className="flex gap-2">
                          <Quote className="h-3 w-3 mt-0.5 text-muted-foreground shrink-0" />
                          <div className="italic text-foreground/90">{ev.quote}</div>
                        </div>
                      )}
                      {ev.reason && (
                        <div>
                          ...
                          <div className="text-foreground/90 mt-0.5">{ev.reason}</div>
                        </div>
                      )}
                    </div>
                  ))}
          ...
          {canAct && (
            <div className="flex flex-col gap-2 shrink-0">
              <Button size="sm" variant="outline" onClick={onResolve} className="gap-1.5">
