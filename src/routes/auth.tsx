        ...
        <div className="halftone-hero relative hidden overflow-hidden rounded-[40px] p-10 lg:flex lg:flex-col lg:justify-between">
          ...
          <div>
            <div className="font-serif text-[color:var(--chalk)] text-[clamp(56px,7vw,96px)] leading-[0.94] tracking-[0.02em]">
              ENTER THE<br/>WRITERS'<br/>ROOM
            </div>
            <p className="mt-6 max-w-sm text-[color:var(--chalk)]/85 text-[16px] leading-[1.55]">
              Living Story DNA. Continuity guardrails. One workspace for every universe you'll ever build.
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center justify-center rounded-[40px] bg-[color:var(--limestone)] p-8 lg:p-14">
          <div className="w-full max-w-md">
            ...
            <p className="mt-3 text-[16px] text-[color:var(--obsidian)]/70">
              {tab === "in" ? "Sign in to your writers' room." : "Create your account. Confirm via email."}
            </p>
              ...
              {(["in", "up"] as const).map((v) => (
                <button
                  ...
                  className={
                    ...
                    (tab === v ? "bg-[color:var(--obsidian)] text-[color:var(--chalk)]" : "text-[color:var(--obsidian)]")
                  }
                >
...
const pillLabel = "block mb-2 text-[12px] font-medium uppercase tracking-wide text-[color:var(--obsidian)]/60";
const pillSubmit =
...
function SignInForm() {
  ...
  return (
    ...
    >
      <div>
        <label className={pillLabel} htmlFor="e">Email</label>
        <input id="e" className={pillInput} type="email" required value={email} onChange={(e) => setEmail(e.target.value)} />
      </div>
      <div>
        <label className={pillLabel} htmlFor="p">Password</label>
        <input id="p" className={pillInput} type="password" required value={password} onChange={(e) => setPassword(e.target.value)} />
      </div>
  ...
  if (sent) {
    return (
      <div className="space-y-2 rounded-[24px] bg-[color:var(--sulfur)] p-6 text-[14px] text-[color:var(--obsidian)]">
        ...
        <p>We sent a confirmation link to <strong>{email}</strong>. Confirm to unlock your creator dashboard.</p>
      </div>
    );
  }
  return (
    ...
    >
      <div>
        <label className={pillLabel} htmlFor="dn">Display name</label>
        <input id="dn" className={pillInput} required value={displayName} onChange={(e) => setDisplayName(e.target.value)} placeholder="e.g. Ada Wren" />
      </div>
      <div>
        <label className={pillLabel} htmlFor="ue">Email</label>
        <input id="ue" className={pillInput} type="email" required value={email} onChange={(e) => setEmail(e.target.value)} />
      </div>
      <div>
        <label className={pillLabel} htmlFor="up">Password</label>
        <input id="up" className={pillInput} type="password" required minLength={6} value={password} onChange={(e) => setPassword(e.target.value)} />
      </div>
      <button type="submit" className={pillSubmit} disabled={busy}>
